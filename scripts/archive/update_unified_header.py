...
<head>
  ...
  <style>
    ...
    /* Pulsing Neon Promo Badge Button */
    @keyframes neonGlow {
...
<header class="unified-header">
  <div class="header-container">
    ...
    <!-- Center: Pulsing Neon Promo Badge Button -->
    {% if store and store.promo_banner_text %}
    <button type="button" class="neon-promo-badge-btn" data-bs-toggle="modal" data-bs-target="#promoBannerModal">
      <span class="neon-badge-pulse"></span>
      <span class="neon-badge-tag">🔥 KATTA AKSIYA</span>
      <span class="d-none d-md-inline opacity-90">{{ store.promo_banner_text|strip_tags|truncatechars:45 }}</span>
      <i class="bi bi-arrow-right-short text-warning font-16 ms-1"></i>
    </button>
    {% endif %}

    <!-- Center-Right: Navigation Links (For Superuser / Admin) -->
    {% if user.is_superuser %}
    <div class="header-links">
      <a href="{% url 'terminal' %}" class="header-link-item {% if request.resolver_match.url_name == 'terminal' %}active{% endif %}">
        <i class="bi bi-display"></i> Savdo Terminali
      </a>
      <a href="{% url 'ai_assistant_page' %}" class="header-link-item {% if request.resolver_match.url_name == 'ai_assistant_page' %}active{% endif %}">
        <i class="bi bi-cpu"></i> AI Yordamchi
      </a>
      <a href="{% url 'customers' %}" class="header-link-item {% if request.resolver_match.url_name == 'customers' %}active{% endif %}">
        <i class="bi bi-people"></i> Mijozlar
      </a>
      <a href="{% url 'article_list' %}" class="header-link-item {% if request.resolver_match.url_name == 'article_list' %}active{% endif %}">
        <i class="bi bi-box-seam"></i> Ombor
      </a>
      
      <!-- More Dropdown -->
      <div class="dropdown d-inline-block">
        <button class="header-link-item btn btn-link text-decoration-none dropdown-toggle border-0" type="button" data-bs-toggle="dropdown">
          <i class="bi bi-grid-3x3-gap"></i> Boshqa
        </button>
        <ul class="dropdown-menu dropdown-menu-end shadow-md mt-2" style="border-radius: 14px; border: 1px solid rgba(0,0,0,0.08);">
          <li><a class="dropdown-item py-2" href="{% url 'daily_report' %}"><i class="bi bi-calendar-check me-2 text-info"></i> Kunlik Hisobot</a></li>
          <li><a class="dropdown-item py-2" href="{% url 'global_analytics' %}"><i class="bi bi-graph-up-arrow me-2 text-primary"></i> Kengaytirilgan Analitika</a></li>
          <li><a class="dropdown-item py-2" href="{% url 'cash_flow' %}"><i class="bi bi-cash-coin me-2 text-success"></i> Kirim-Chiqim</a></li>
          <li><a class="dropdown-item py-2" href="{% url 'yield_loss' %}"><i class="bi bi-calculator me-2 text-warning"></i> Tannarx</a></li>
          <li><a class="dropdown-item py-2" href="{% url 'debt_payment' %}"><i class="bi bi-wallet2 me-2 text-danger"></i> Qarz to'lovi</a></li>
          <li><a class="dropdown-item py-2" href="{% url 'suppliers_dashboard' %}"><i class="bi bi-truck me-2 text-dark"></i> Ta'minotchilar</a></li>
          <li><hr class="dropdown-divider"></li>
          <li><a class="dropdown-item py-2" href="{% url 'article_new' %}"><i class="bi bi-plus-circle me-2 text-warning"></i> Yangi Go'sht qo'shish</a></li>
        </ul>
      </div>
    </div>
    {% endif %}

    <!-- Right: Utility Controls (Script Switcher, Phone, User Profile) -->
    <div class="header-actions">
      ...
      <!-- Phone Link -->
      <a href="tel:+998770824477" class="d-none d-xl-inline text-white-50 font-12 text-decoration-none" style="font-size: 12px;">
      ...
      <div class="dropdown">
        <button class="btn btn-sm btn-link text-decoration-none dropdown-toggle p-0 d-flex align-items-center gap-2 text-white" type="button" data-bs-toggle="dropdown">
        ...
        <ul class="dropdown-menu dropdown-menu-end shadow-sm" style="border-radius: 14px; padding: 8px;">
          <div class="px-3 py-2 border-bottom mb-2">
            <div class="fw-bold" style="font-size: 13px;">{{ user.get_full_name|default:user.username }}</div>
            <div class="text-muted" style="font-size: 11px;">{% if user.is_superuser %}Administrator{% else %}Mijoz{% endif %}</div>
          </div>
          {% if user.is_superuser %}
          <li><a class="dropdown-item rounded-2 py-2" href="/admin/"><i class="bi bi-database-check me-2"></i> Django Admin</a></li>
          {% else %}
          <li><a class="dropdown-item rounded-2 py-2" href="/pos/my-cabinet/"><i class="bi bi-person-vcard me-2"></i> Shaxsiy Kabinet</a></li>
          {% endif %}
          <li><hr class="dropdown-divider"></li>
          <li>
            <form action="{% url 'logout' %}" method="post" class="m-0">
              {% csrf_token %}
              <button type="submit" class="dropdown-item text-danger rounded-2 py-2"><i class="bi bi-box-arrow-right me-2"></i> Chiqish</button>
            </form>
          </li>
        </ul>
      </div>
...
<div id="mobile-navbar-dropdown" class="mobile-menu-drawer">
  <div class="d-flex flex-column gap-2">
    {% if user.is_superuser %}
    <a href="{% url 'terminal' %}" class="header-link-item"><i class="bi bi-display"></i> Savdo Terminali</a>
    <a href="{% url 'ai_assistant_page' %}" class="header-link-item"><i class="bi bi-cpu"></i> AI Yordamchi</a>
    <a href="{% url 'customers' %}" class="header-link-item"><i class="bi bi-people"></i> Mijozlar Bazasi</a>
    <a href="{% url 'customer_chats_dashboard' %}" class="header-link-item"><i class="bi bi-chat-dots"></i> Mijozlar Chati</a>
    <a href="{% url 'article_list' %}" class="header-link-item"><i class="bi bi-box-seam"></i> Ombor</a>
    {% else %}
...
print("SUCCESSFULLY_UPDATED_UNIFIED_HEADER")
